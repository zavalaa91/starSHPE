...
def checkIfExists(myUIN, myCursor):
    ...
    for storedUIN in myCursor.fetchall():
        if str(storedUIN)[1:-3] == str(myUIN):
            return True
    return False
...
def connectToDB():
    ...
    '''
    db=MySQLdb.connect(host='174.132.129.8',user='shpeuiuc_2012',passwd='xtTjlqno',db='shpeuiuc_members',port=3306)
    myCursor = db.cursor()
    return myCursor
...
def firstAttendance(UIN, myCursor):
    ...
    @param myCursor: a cursor for the SHPE members database.
    '''
    myCursor.execute('INSERT INTO members (UIN,gbmsAttended) VALUES (%d,%d)' % (UIN,1))
